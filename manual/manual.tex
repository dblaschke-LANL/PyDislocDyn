...
All entries adhere to the format\footnote{As of PyDislocDyn 1.4.0, the .toml and .yaml formats are also supported.}
\\\verb|keyword = value|\\
...
\end{itemize}
\noindent
The following keywords are optional (depending on the intended use case)\footnote{Legacy keywords `\texttt{burgers}', `\texttt{b}', and `\texttt{n0}' for input files are deprecated in PydislocDyn 1.4.0, users should use Miller indices instead.}:
\begin{itemize}
...
\begin{itemize}
   ...
   \item function \verb|read_dislocdyn_output(fname)| can be used to read an output file of the Fortran frontend, \verb|fname| (PyDislocDyn 1.4.0 and up).
