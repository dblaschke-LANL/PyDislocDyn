...
PyDislocDyn \cite{pydislocdyn,Blaschke:2025joss} is a suite of python programs designed to perform various calculations for basic research in dislocation dynamics in metals with various crystal symmetries in the continuum limit.
...
\item compute theoretical limiting velocities of dislocations in arbitrary slip systems and with arbitrary character angle \cite{Blaschke:2026vlim,Blaschke:2021vcrit,Blaschke:2020MD,Teutonico:1961,Barnett:1973b}
\item compute the elastic strain energy and line tension for any dislocation \cite{Blaschke:2017lten}
\item compute the dislocation drag coefficient $B$ from phonon wind for any dislocation at any velocity \cite{Alshits:1979,Alshits:1992,Blaschke:BpaperRpt,Blaschke:2018anis,Blaschke:2019fits,Blaschke:2019Bpap} or stress \cite{Blaschke:2019a,Blaschke:2021impact,Blaschke:2021temperature}.
\end{itemize}

\noindent
This manual is intended as a brief introduction on how to use the most common features of PyDislocDyn 1.4.x \cite{pydislocdyn,Blaschke:2025joss}.
...
\begin{itemize}
\item PyDislocDyn 1.4.0 and higher requires Python 3.10 or higher\footnote{
The last version to support Python 2.7 was PyDislocDyn 1.2.0, Python 3.5 was supported up to version 1.2.3, Python 3.6 up to version 1.2.4, Python 3.8 up to version 1.2.9, and Python 3.9 up to 1.3.5.}.
As of PyDislocDyn 1.4.0, required modules include Numpy $\ge1.26$, Scipy $\ge1.10$, Sympy $\ge1.12$, Matplotlib $\ge3.8$, Pandas $\ge1.5$ (including its optional dependency Jinja2), and mpmath (recommended optional dependency of mpmath: gmpy).
On Python 3.10, tomli is an additional requirement.
\item Optional, but highly recommended in order to conduct drag coefficient calculations with reasonable speed are:\\
Meson\footnote{%
Note: building the Fortran subroutines of previous PyDislocDyn 1.3.x releases with Numpy $\le1.26$ requires Setuptools $<70$.%
} and a Fortran compiler supported by numpy.f2py to compile the optional Fortran subroutines (resp. a Fortran 2018 compiler, such as gfortran>=10, for the standalone Fortran library and frontend), Joblib $\ge1.1$ (for parallelization),
threadpoolctl>=3.3 (for automatically adjusting the number of OpenMP threads in the Fortran subroutines and Numpy to avoid over-committing if Joblib is used), and 
Numba $\ge0.53$ (a just-in-time compiler for Python; the Fortran subroutines are faster and thus preferred however).
\\
To compile the subroutines of PyDislocDyn $\ge1.3.0$, simply run\\
 \verb|python -c 'import pydislocdyn; pydislocdyn.utilities.compilefortranmodule()|'\\
See the readme file for additional information on installing PyDislocDyn and compiling its subroutine module as well as the standalone Fortran library included in PyDislocDyn >=1.4.0.
...
Unless set by the user prior to running Python in the terminal, PyDislocDyn will attempt to choose an optimal value\footnote{
Note: this may fail if threadpoolctl is not installed and a recent version of numpy is imported before pydislocdyn.}.
Furthermore, PyDislocDyn's joblib parallelization will automatically adjust its `Ncores' variable to avoid over-committing.
\end{itemize}


\section{Running the Python code}
...
The latter are assembled in dictionaries within `\verb|metal_data.py|', see Refs. \cite{Hertzberg:2012,Kaye:2004,CRCHandbook,Wasserbaech:1990,Seeger:1960,Graham:1968,Smith:1966,Kiewel:1996,Samsonov:1968,Epstein:1965,Lowrie:1967,Thomas:1968,Hiki:1966,Leese:1968,Voronov:1978,Alers:1960,Saunders:1986,Powell:1984,Naimon:1971,Riley:1973,Swartz:1972,Rao:1973,Swartz:1970,Singh:1992,Barrera:1993,Jiles:1981,Yogurtcu:1985,Srinivasan:1973a,Rao:1980,Krasilnikov:2012,Vekilov:2016,Suzuki:1971,Cai:2020,Duzgun:1993,Blind:1979}.
...
PyDislocDyn 1.4.x input files are toml files assigning values to keywords known to PyDislocDyn.
All numerical values must be set in SI units.
...
\item lattice constant(s) and angles must be in the \verb|[lattice]| section of the toml file: lattice constant \verb|a| is always required, \verb|b| and \verb|c| may be required depending on \verb|sym|; angles between lattice basis vectors (only required by lower symmetries where they cannot be inferred from keyword \verb|sym|) are denoted by \verb|alpha|, \verb|beta|, \verb|gamma| and are provided in degrees (not radians).
The unit cell volume is then automatically computed.

\item Second order elastic constants (SOEC) must be in the \verb|[soec]| section of the toml file: for \verb|sym| in one of \verb|iso|--\verb|tetr|, every required elastic constant may be set with its own keyword (e.g. \verb|c44| etc.);
...
\item The slip system is defined using Miller indices in the \verb|[slip]| section of the toml file:
\begin{itemize}
\item \verb|Millern0|: requires comma separated values and denotes the slip plane normal in Miller indices for planes (i.e. in reciprocal space); its length is ignored, Miller indices \verb|Millern0| are automatically converted to Cartesian coordinates \verb|n0|, and \verb|n0| is automatically normalized.

\item \verb|Millerb|: requires comma separated values and denotes the Burgers vector in Miller indices; its length is used to infer the Burgers vector length \verb|burgers|.
Miller indices \verb|Millerb| are automatically converted to Cartesian coordinates \verb|b|, and \verb|b| is automatically normalized.
\end{itemize}
\end{itemize}
\noindent
The following keywords are optional (depending on the intended use case)\footnote{Legacy keywords `\texttt{burgers}' (for overriding the Burgers vector length), `\texttt{b}' (Burgers vector in Cartesian coordinates), and `\texttt{n0}' (slip plane normal in Cartesian coordinates) for input files are deprecated in PyDislocDyn 1.4.0, users should use Miller indices instead.}:
\begin{itemize}
...
\item \verb|alpha_a|: denotes the thermal expansion coefficient; only used if dislocation drag is computed for multiple temperatures $\ge$T.

\item \verb|lam| and \verb|mu| may be provided in the \verb|[soec]| section of the toml file: polycrystal averages for the Lam\'e constants; if omitted, they are calculated by averaging over the (anisotropic) SOECs.

\item Third order elastic constants (TOEC) can be provided in a \verb|[toec]| section in the toml file, these are required for phonon wind (dislocation drag) calculations:
for \verb|sym| in one of \verb|iso|--\verb|tetr|, every required elastic constant may be set with its own keyword (e.g. \verb|c123| etc.);
...
\end{itemize}
The same files can also be used as material files for the Fortran frontend, although the slip systems will be ignored and have to be provided in a separate inputdeck as described below in Section \ref{sec:fortran}.
...
\item \verb|.computesound(v)| to calculate the sound speeds for a wave propagating in direction v, see e.g. \cite{Fitzgerald:1967}
...
Instances of this class can be populated using the same input file format discussed above in section \ref{sec:inputfiles}, though keywords relating to dislocations (i.e. \verb|b|, \verb|n0|, \verb|Millerb|, \verb|Millern0|, \verb|burgers|) must be omitted.
In order to initialize an instance of the \verb|metal_props| class from an input file (that does not define a slip system), use function `\verb|pydislocdyn.readinputfile()|'.
...
\begin{itemize}
	...
	\item \verb|elasticC2| and \verb|elasticC3| to generate the tensors (i.e. the multidimensional numpy arrays) of second and third order elastic constants in Voigt notation (default) or in Cartesian coordinates (set \verb|voigt=False|).
	\item \verb|elasticS2| and \verb|elasticS3| to calculate the second and third order compliance tensors from the elastic (or stiffness) tensors in Voigt notation (default) or in Cartesian coordinates (set \verb|voigt=False|).
   ...
   \item the \verb|metal_props| class, see Section \ref{sec:metal_props}; function \verb|readinputfile()| will fall back to populating an instance of this class from an input file if no slip system is defined in that file.
   ...
   \item function \verb|read_dislocdyn_output(fname)| can be used to read an output file of the Fortran frontend, \verb|fname| (PyDislocDyn 1.4.0 and up, see Section \ref{sec:fortran} below).
   It returns a dictionary containing pandas DataFrames for limiting velocities and/or drag coefficients for each material found in \verb|fname|.
\end{itemize}
...
\section{Running the Fortran code}
\label{sec:fortran}

PyDislocDyn >=1.4.0 includes a Fortran library, \verb|dislocdynlib|, and a frontend, \verb|dislocdyn|.
The recommended way to compile both is via the Fortran Package Manager (fpm);
Use \verb|ford| to generate the detailed documentation of the library API.
The Fortran frontend is input file driven and requires as its commandline arguments one or more material files as well as an inputdeck:
\\\verb|fpm run --profile release -- <one or more material files> <inputdeck>|\\
The material files have exactly the same (toml) format as the input files for the Python code as described in Section \ref{sec:inputfiles}, except that the slip planes therein are ignored and must be provided inside the input deck (also a \verb|.toml|-file) instead.
%All entries adhere to the toml format: 
%\verb|keyword = "value"|.\\
At the very least the input deck must define a slip plane via the \verb|Millerb| (or \verb|b|) and \verb|Millern0| (or \verb|n0|) keywords and specify what to calculate via the \verb|sim_type| keyword.
In particular, 
\\\verb|sim_type = "vlimit"|\\
will instruct \verb|dislocdyn| to calculate dislocation limiting velocities and
\\\verb|sim_type = "drag"|\\
will instruct \verb|dislocdyn| to calculate the dislocation drag coefficient from phonon wind.
To do both, just add a second entry to the input deck.
The Burgers vector direction \verb|b| and slip plane normal \verb|n0| must be provided as Miller indices and the Burgers vector length is determined from vector \verb|b|.
Note that the Fortran version currently does not support reading fractions from the input deck.
Instead, keyword \verb|Millernorm| may be used to set a number to divide all entries of \verb|b| by.
Other keywords that may be set in the input deck, but have default values if they are not specified, include:
\begin{itemize}
\item \verb|ntheta| $\ldots$ number of dislocation character angles to consider between 0 (screw) and $\pi/2$ (edge). The default is 2 (only screw and edge).
\item \verb|include_negative_theta| $\ldots$ whether or not to include negative dislocation characters (default=false). Many slip systems are symmetric with respect to the sign of character angle for mixed dislocations, and it is up to the user to decide if negative character angles need to be considered separately.
In contrast to the Python version, the Fortran derived type for dislocations is restricted to character angles from 0 to $\pi/2$, but all signs can be flipped.
Therefore, under the hood, the frontend will initialize two dislocation variables, one for the positive and one for the negative angles if \verb|include_negative_theta = true| in the input deck.
\item \verb|nphi| $\ldots$ resolution of polar angles to use for the dislocation field (defaults to 500)
\item \verb|logfile| $\ldots$ name of the file to write to (defaults to \verb|dislocdyn.log|)
\item \verb|betamin| $\ldots$ smallest dislocation gliding velocity in units of average transverse sound speed $c_T$ to consider in dislocation drag calculations (defaults to 0.01)
\item \verb|betamax| $\ldots$ largest dislocation gliding velocity in units of $c_T$ to consider in dislocation drag calculations (defaults to 0.99)
\item \verb|nbeta| $\ldots$ number of  dislocation gliding velocities to consider between \verb|betamin| and \verb|betamax| (defaults to 1, meaning only \verb|betamin| is calculated)
\end{itemize}
The instructions in the input deck apply to all material files included in a run, i.e. limiting velocities and/or drag coefficients are computed for all material files in order of appearance on the command line.
Since the slip plane is defined in the input deck, an important limitation is that all material files included in a run must have the same crystal symmetry \verb|sym|.

For the drag coefficient calculations, the Fortran version prioritizes speed over accuracy and therefore uses the high temperature Debye expansion of the phonon distributions to skip one of the numeric integrations.
Also, the adaptive grid present in the Python version is not implemented in the Fortran code at this point.
Therefore, the Fortran results will match the Python results only if the latter's \verb|phonondrag()| function is called with options \verb|Debye_series=True| and \verb|maxrec=-1|.
