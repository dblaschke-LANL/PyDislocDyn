...
\noindent
This manual is intended as a brief introduction on how to use the most common features of PyDislocDyn 1.2.9 and higher \cite{pydislocdyn}.
...
Since pydislocdyn (from version 1.2.9) is a `regular package' in python, all front-end scripts can also be run via python's -m option, e.g. in the present case:\\
...
\\\verb|`from pydislocdyn.polycrystal_averaging import readinputfile'|
...
\begin{itemize}
   ...
   \item the \verb|metal_props| class, see Section \ref{sec:metal_props}; a function to populate an instance from an input file is available from sub-module \verb|polycrystal_averaging.readinputfile|.
...
\begin{itemize}
	...
	\item \verb|pydislocdyn.dislocations| which contains the \verb|StrohGeometry| class (one of two parents of the \verb|Dislocation| class) and functions used under the hood by the \verb|Dislocation| class.
	...
	\item the front-end scripts which are described in Sec. \ref{sec:frontends}, \verb|pydislocdyn.polycrystal_averaging|,  \verb|pydislocdyn.linetension_calcs|, and \verb|pydislocdyn.dragcoeff_semi_iso|, are also sub-modules; classes and functions useful to the user are aliased to the top level and are thus available directly under \verb|pydislocdyn|, as listed above.
\end{itemize}
