...
} and a Fortran compiler supported by numpy.f2py to compile the optional Fortran subroutines, Joblib $\ge1.1$ (for parallelization),
threadpoolctl (for automatically adjusting the number of OpenMP threads in the Fortran subroutines and Numpy to avoid over-committing if Joblib is used), and 
...
\\
%Note: building the Fortran subroutines with Numpy $\le1.26$ requires Setuptools $<70$, whereas building with Numpy $\ge2.0$ / Python $\ge3.12$ requires Meson (see the Numpy documentation for details).
