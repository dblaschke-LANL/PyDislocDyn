    ...
    if len(sys.argv) > 1:
        ...
        try:
            for i in range(len(args)):
                inputdata[i]=readinputfile(args[i],init=False)
                X = inputdata[i].name
                metal_list.append(X)
                Y[X] = inputdata[i]
            use_metaldata=False
