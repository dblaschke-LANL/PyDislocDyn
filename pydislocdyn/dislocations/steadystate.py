...
if usefortran:
    ...
    fourieruij_sincos = dislocdyn_phononwind.fourieruij_sincos
    ...
    def fourieruij_nocut(uij,phiX,sincos,Ntheta,phres):
        ...
        return dislocdyn_phononwind.fourieruij_nocut(np.moveaxis(uij,-1,0),phiX,sincos,Ntheta,phres)
    ...
    def computeEtot(uij, betaj, C2, Cv, phi):
        ...
        return dislocdyn_subroutines.computeetot(np.moveaxis(uij,-1,0),betaj,C2,Cv,phi)
    ...
    def elbrak(A,B,elC):
        ...
        return np.moveaxis(dislocdyn_utilities.elbrak(np.moveaxis(A,-1,0),np.moveaxis(B,-1,0),elC),0,-1)
    ...
    def elbrak1d(A,B,elC):
        ...
        return dislocdyn_utilities.elbrak1d(A,B,elC)
    
    def computeuk(beta, C2, Cv, b, M, N, phi, r):
        '''Compute the dislocation displacement field uk according to the integral method.'''
        return np.moveaxis(dislocdyn_subroutines.computeuk(beta, C2, Cv, b, np.moveaxis(M,-1,0), np.moveaxis(N,-1,0), phi, r),0,-1)
    ...
    def computeuij(beta, C2, Cv, b, M, N, phi, r=None, debug=False):
        ...
        out = np.moveaxis(dislocdyn_subroutines.computeuij(beta, C2, Cv, b, np.moveaxis(M,-1,0), np.moveaxis(N,-1,0), phi),0,-1)
        if r is not None:
            Ntheta = len(M[0,:,0])
