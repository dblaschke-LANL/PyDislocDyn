    ...
    def _computevcrit_object(self):
        ...
        if self.sym=='iso' or indices[0] is not None:
            ...
            if vcrit_screw is not None:
                vcrit_screw = sp.sqrt(sp.simplify(roundcoeff(sp.simplify(vcrit_screw**2))))
            else:
...
def readinputfile(fname,init=True,theta=None,Nphi=500,Ntheta=2,symmetric=True,isotropify=False,include_extra=False):
    ...
    if theta is None:
        ...
        if include_extra:
            ...
            Ntheta = len(theta)
            theta = np.linspace(theta[0]-dtheta,theta[-1]+dtheta,Ntheta+2)
