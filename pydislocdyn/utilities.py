...
if sys.version_info < (3,11):
    ## if we're running python 3.10, fall back to tomlkit:
...
def loadinputfile(fname,optionmode=False):
    ...
    Alternatively, we also support the toml and yaml file formats (auto-detected based on the file endings).
    '''
    inputparams={}
    if str(fname)[-5:] == ".toml":
        with  open(fname,"rb") as f1:
            inputparams = tomllib.load(f1)
        return inputparams
...
def dumpinputfile(data,fname):
    """
    Writes an input file based on a dictionary using one of the supported file formats inferred from the file ending of 'fname'.
    Supported formats currently are: '.toml', '.yaml' (or '.yml'; requires pyyaml), or our default/legacy format for any other ending.
    Use dumpinputfile(loadinputfile(fname),newfname)) to convert an existing input file
    to a different format (warning: comments are not preserved).
    """
    if str(fname)[-5:] == ".toml":
