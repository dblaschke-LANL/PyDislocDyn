...
def prepare_testfolder(old,new,verbose=True):
...
def test_aver(old=baseln,new=cwd,skip_calcs=False,verbose=True):
...
def test_dragiso(old=baseln,new=cwd,skip_calcs=False,verbose=True,metals='Cu Fe',**kwargs):
    ...
    if metals == 'all':
        if opts['use_exp_Lame']:
            metals_temp = sorted(ISO_l.keys())
        else:
            metals_temp = sorted(c111.keys())
        metals=''
        for i in metals_temp:
            metals += i+' '
        metals = metals.strip()
    commandargs.append('--allplots') # want to test also the plots that are not generated by default
...
def test_drag(old=baseln,new=cwd,skip_calcs=False,verbose=True,metals='Al Mo Ti Sn',**kwargs):
    ...
    commandargs.append('--allplots') # want to test also the plots that are not generated by default
    commandargs.append(f'{metals}')
    metals = expand_slipsystems(metals,bccslip=opts['bccslip'],hcpslip=opts['hcpslip'],tetrslip=None)
...
def test_LT(old=baseln,new=cwd,skip_calcs=False,verbose=True,metals='Al Mo Ti Sn',**kwargs):
    ...
    if not skip_calcs:
        ...
        filelist = sorted(expand_slipsystems(metals,bccslip=opts['bccslip'],hcpslip=opts['hcpslip'],tetrslip=None))
        filelist = [pathlib.Path(os.pardir,"temp_pydislocdyn")/(i+".toml") for i in filelist]
...
def test_acc_screw(old=baseln,new=cwd,skip_calcs=False,verbose=True,metals='Al Mo Ti Sn',**kwargs):
    ...
    if not skip_calcs:
        ...
        for X in metal_acc_screw:
            acc_screw[X] = readinputfile(pathlib.Path(tmppydislocdyn,X+".toml"),Nphi=50)
...
def test_acc_edge(old=baseln,new=cwd,skip_calcs=False,verbose=True,metals='Al Mo Ti Sn',**kwargs):
    ...
    if not skip_calcs:
        ...
        for X in metal_acc_edge:
            acc_edge[X] = readinputfile(pathlib.Path(tmppydislocdyn,X+".toml"),Nphi=25)
...
def test_misc(old=baseln,new=cwd,skip_calcs=False,verbose=True,metals='all',**kwargs):
    ...
    if not skip_calcs:
        ...
        def maincomputations(X):
            Y = readinputfile(pathlib.Path(tmppydislocdyn,X+".toml"),Ntheta=5)
            Y.computevcrit()
...
def test_strainpoly(old=baseln,new=cwd,skip_calcs=False,verbose=True,**kwargs):
