...
def test_fortran_drag_fcc(rnd=6,beta=0.25,Ntheta=3):
    ...
    for X in sorted(pydislocdyn.metal_data.fcc_metals):
        Y[X] = pydislocdyn.readinputfile(tmpfolder / (X+".toml"),Ntheta=Ntheta)
...
def test_fortran_drag_bcc(rnd=6,beta=0.25,Ntheta=3):
    ...
    for Xm in sorted(pydislocdyn.metal_data.bcc_metals):
        for slip in ["110", "112", "123"]:
            ...
            if slip in ['110','123']:
                symmetric = False
            Y[X] = pydislocdyn.readinputfile(tmpfolder / (X+".toml"),Ntheta=Ntheta,symmetric=symmetric)
            if Xm in aver_lame:
...
def test_fortran_drag_hcp(rnd=6,beta=0.25,Ntheta=3):
    ...
    for Xm in sorted(pydislocdyn.metal_data.hcp_metals):
        for slip, slipL in hcpslip.items():
            X = Xm+slip
            Y[X] = pydislocdyn.readinputfile(tmpfolder / (str(Xm+slipL)+".toml"),Ntheta=Ntheta)
            drag_py[X] = pydislocdyn.phonondrag(Y[X],beta,maxrec=-1,Debye_series=True).round(frnd)
            # compare results
...
def test_fortran_drag_tetr(rnd=6,beta=0.25,Ntheta=3):
    ...
    for islip in range(10):
        ...
        for X in sorted(pydislocdyn.metal_data.bct_metals):
            command.append(tmpfolder / (X+slip+".toml"))
        fname = f"drag_tetr_bct{slip}.toml"
        with open(fname,"w",encoding="utf8") as infile:
            infile.write(f'sim_type = "drag"\nntheta = {Ntheta}\nlogfile = "{fname[:-4]}log"\nechoinput = true\n')
            infile.write(f"betamin = {beta}\nnbeta = 1\n")
        ...
        with open(f"{fname[:-4]}log", 'w', encoding="utf8") as logfile:
            with subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True) as subproc:
                for line in subproc.stdout:
                    logfile.write(line)
                subproc.wait()
        drag_f['bct'+slip] = read_dislocdyn_output(f"{fname[:-4]}log",postprocess=True)
    # fct metals:
    for islip in range(3):
        ...
        for X in sorted(pydislocdyn.metal_data.fct_metals):
            command.append(tmpfolder / (X+slip+".toml"))
        fname = f"drag_tetr_fct{slip}.toml"
        with open(fname,"w",encoding="utf8") as infile:
            infile.write(f'sim_type = "drag"\nntheta = {Ntheta}\nlogfile = "{fname[:-4]}log"\nechoinput = true\n')
            infile.write(f"betamin = {beta}\nnbeta = 1\n")
        ...
        with open(f"{fname[:-4]}log", 'w', encoding="utf8") as logfile:
            with subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True) as subproc:
                for line in subproc.stdout:
                    logfile.write(line)
                subproc.wait()
        drag_f['fct'+slip] = read_dislocdyn_output(f"{fname[:-4]}log",postprocess=True)
